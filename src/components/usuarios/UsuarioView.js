...
import { postUsuario, getUsuarios, putUsuario } from '../../services/usuarioService'
import Modal from '../ui/ModalUsu'
import ModalEdit from '../ui/ModalUsuEdit'
...
export const UsuarioView = () => {
  ...
  const [tipoEquipo, setTipoEquipo] = useState({nombre: '', email: '', estado: 'Inactivo'})
  ...
  const saveTipoEquipo = async () => {
    try{
      setError(false)
      setLoadingSave(true)
      const response = await postUsuario(tipoEquipo)
  ...
  const closeModal = () => {
    setTipoEquipo({nombre: '', email:'', estado:'Inactivo'})
    if(id)setId('')
  }
  ...
  const editTipoEquipo = async () => {
    try{
      setError(false)
      setLoadingSave(true)
      const response = await putUsuario(id, tipoEquipo)
      console.log(response)
      setTipoEquipo({nombre: '', email: ''})
      listTipoEquipos()
      setTimeout(() => {
