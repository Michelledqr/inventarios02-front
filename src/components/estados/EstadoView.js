...
import { postEstadoEquipo, getEstadosEquipo, putEstadoEquipo } from '../../services/estadoEquipoService'
...
export const EstadoView = () => {
  ...
  const [tipoEquipo, setTipoEquipo] = useState({nombre: '', estado: 'Inactivo'})
  ...
  const listTipoEquipos = async () => {
    try{
      setError(false)
      setLoading(true)
      const { data } = await getEstadosEquipo(query)
      setTipoEquipos(data)
      setTimeout(() => {
        setLoading(false)
      }, 500)
  ...
  const saveTipoEquipo = async () => {
    try{
      setError(false)
      setLoadingSave(true)
      const response = await postEstadoEquipo(tipoEquipo)
      console.log(response)
      setTipoEquipo({nombre: '', estado: ''})
      listTipoEquipos()
      setTimeout(() => {
  ...
  const closeModal = () => {
    setTipoEquipo({nombre: '', estado:'Inactivo'})
    if(id)setId('')
  }
  ...
  const editTipoEquipo = async () => {
    try{
      setError(false)
      setLoadingSave(true)
      const response = await putEstadoEquipo(id, tipoEquipo)
      console.log(response)
