...
import { postTipoEquipo, getTiposEquipo, putTipoEquipo } from '../../services/tipoEquipoService'
...
export const TipoView = () => {
  ...
  const listTipoEquipos = async () => {
    try{
      setError(false)
      setLoading(true)
      const { data } = await getTiposEquipo(query)
      setTipoEquipos(data)
      setTimeout(() => {
        setLoading(false)
      }, 500)
  ...
  const saveTipoEquipo = async () => {
    try{
      ...
      console.log('data antes grabar', tipoEquipo);

      const response = await postTipoEquipo(tipoEquipo)
  ...
  const editTipoEquipo = async () => {
    try{
      setError(false)
      setLoadingSave(true)
      const response = await putTipoEquipo(id, tipoEquipo)
      console.log(response)
      setTipoEquipo({nombre: ''})
      listTipoEquipos()
      setTimeout(() => {
